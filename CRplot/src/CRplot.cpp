...
void CRplot::plot(nkk::State current_state, std::vector<nkk::Input> captureRegion)
{
	fprintf(p.gp, "plot");

	if (!captureRegion.empty())
	{
		sort(captureRegion.begin(), captureRegion.end(), [](const Input & a, const Input & b) {
			return a.n > b.n;
		}); //descending
		...
		while (step_num != 0)
		{
			...
			{
				fprintf(p.gp, "%f %f\n", captureRegion[ind].dsf.th, captureRegion[ind].dsf.r);
				ind++;
			}
			fprintf(p.gp, "e\n");
			step_num--;

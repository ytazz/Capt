...
__device__ Grid::Grid(Param param) : param(param) {
  state.clear();
  input.clear();
...
__device__ int Grid::round(float value) {
...
__device__ int Grid::max(int val1, int val2) {
  int max_val = 0;
...
__device__ int Grid::max(int val1, int val2, int val3, int val4) {
  int max_val = 0;
...
__device__ GridState Grid::roundState(State state_) {
  using GridSpace::MAX;
  using GridSpace::MIN;
  using GridSpace::STEP;
...
__device__ int Grid::getStateIndex(State state_) {
  int id = -1;
...
__device__ void Grid::setStatePolar(float icp_r, float icp_th, float swft_r,
                                    float swft_th) {
  State state_;
...
__device__ void Grid::setStateCartesian(float icp_x, float icp_y, float swft_x,
                                        float swft_y) {
  State state_;
...
__device__ void Grid::setInputPolar(float swft_r, float swft_th) {
  Input input_;
...
__device__ void Grid::setInputCartesian(float swft_x, float swft_y) {
  Input input_;
...
__device__ bool Grid::existState(int state_id) {
  bool is_exist = false;
...
__device__ bool Grid::existState(State state_) {
  using GridSpace::MAX;
  using GridSpace::MIN;
  using GridSpace::STEP;
...
__device__ bool Grid::existInput(int input_id) {
  bool is_exist = false;
...
__device__ State Grid::getState(int index) {
  if (!existState(index)) {
...
__device__ int Grid::getStateIndex(int icp_r_id, int icp_th_id, int swft_r_id,
                                   int swft_th_id) {
  int index = 0;
...
__device__ State Grid::getState(int icp_r_id, int icp_th_id, int swft_r_id,
                                int swft_th_id) {
  int index = getStateIndex(icp_r_id, icp_th_id, swft_r_id, swft_th_id);
  return getState(index);
}

__device__ Input Grid::getInput(int index) {
  if (!existInput(index)) {
...
__device__ Input Grid::getInput(int swft_r_id, int swft_th_id) {
  int index = 0;
...
__device__ int Grid::getNumState() { return num_state; }

__device__ int Grid::getNumInput() { return num_input; }

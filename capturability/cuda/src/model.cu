...
__device__ Model::Model(const std::string &name) : Loader(name), pi(M_PI) {
...
__device__ void Model::callbackElement(const std::string &name,
                                       const bool is_start) {
  if (is_start) {
    switch (element) {
    case MODEL_ELE_NONE:
...
__device__ void Model::callbackAttribute(const std::string &name,
...
__device__ std::string Model::getStr(const char *element_name,
...
__device__ float Model::getVal(const char *element_name,
                               const char *attribute_name) {
  float val = 0.0;
...
__device__ std::vector<Vector2> Model::getVec(const char *element_name,
                                              const char *attribute_name) {
...
__device__ float Model::getLinkVal(ELink link, const char *attribute_name) {
  float val = 0.0;
...
__device__ float Model::getLinkVal(int link_id, const char *attribute_name) {
  ELink link = static_cast<ELink>(link_id);
  return getLinkVal(link, attribute_name);
}

__device__ vec3_t Model::getLinkVec(ELink link, const char *attribute_name) {
...
__device__ vec3_t Model::getLinkVec(int link_id, const char *attribute_name) {
  ELink link = static_cast<ELink>(link_id);

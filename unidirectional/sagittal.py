    ...
    def cal_inner(self):
        ...
        for i in range(len(self.F)):
            ...
            for j in range(len(self.L_STRIDE)):
                ...
                F_NEXT = np.array([self.L_STEP_NEXT, self.L_CP_NEXT])
                flag = self.set_squareframe(F_NEXT)
                if flag == 1 and (self.F[i][1] > 0.1 or self.F[i][1] < -0.1):
                    if self.check_frame(self.P):
        ...
        self.P = lexsort_based(P_TEMP)
        self.writefile(data = TEMP, purpose = "sim")
        # self.writefile(data = self.P, purpose =  "plot")

    def writefile(self, data, purpose = "non"):
        np.savetxt('%s_%d_%s.csv' % (self.string, self.count, purpose), data, delimiter=',')
        self.count += 1

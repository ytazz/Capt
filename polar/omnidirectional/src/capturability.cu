...
void makeGridsTable(float cpR[], float cpTh[], float stepR[], float stepTh[]) {
    ...
    for (int i = 0; i < N_CP_R; i++) {
        fprintf(fp, "%lf, %lf, %lf, %lf \n", cpR[i], cpTh[i], stepR[i],
                stepTh[i]);
    }
    fclose(fp);
}

void initializing(Data* dataSet, float cpR[], float cpTh[], float stepR[],
                  float stepTh[]) {
    long int a = 0;
    int      b = 0;
...
__global__ void step_1(Data* dataSet) {
    ...
    while (tid < (N_STATE * N_INPUT)) {
        ...
        oneStepAfterState =
            stepping(dataSet[a].state, dataSet[a].input[b].step);
...
__global__ void step_N(Data* dataSet, int n_step, float* cpR, float* cpTh,
    ...
    while (tid < (N_STATE * N_INPUT)) {
        ...
        if (dataSet[a].input[b].c_r == FAILED) {
            ...
            if (isInPrevSet(dataSet, oneStepAfterState, n_step, cpR, cpTh,
                            stepR, stepTh)) {
...
__device__ bool isInPrevSet(Data* dataSet, State p, int n_step, float cpR[],
                            float cpTh[], float stepR[], float stepTh[]) {
    if (p.icp.r >= CP_MAX_R) {
        return false;
    } else {
        ...
        if (flag[0] < n_step && flag[1] < n_step && flag[2] < n_step &&
            flag[3] < n_step && flag[4] < n_step && flag[5] < n_step &&
            flag[6] < n_step && flag[7] < n_step && flag[8] < n_step &&
            flag[9] < n_step && flag[10] < n_step && flag[11] < n_step &&
            flag[12] < n_step && flag[13] < n_step && flag[14] < n_step &&
            flag[15] < n_step) {
            return true;
        } else {
            return false;
...
__device__ void setBound(State* bound, State p, float cpR[], float cpTh[],
                         float stepR[], float stepTh[]) {
